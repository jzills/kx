...
`events_max_age` is the same kind of duration for `kx events`, and a separate
key on purpose: narrowing the evidence behind a verdict is not a request for a
shorter event listing. Neither key falls back to the other.
